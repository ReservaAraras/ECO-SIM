...
- Differentiation Focus: Mortality (Old Age & Starvation Physics) emphasizing fragmented edge effects.
- Indicator species: Coruja-buraqueira (Athene cunicularia).
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        svg = [f'<svg viewBox="0 0 {w} {h}" xmlns="http://www.w3.org/2000/svg" style="background-color:#0c0808; font-family:system-ui, -apple-system, sans-serif;">']

        svg.append(
            '<defs>'
            '<pattern id="dotGrid" width="40" height="40" patternUnits="userSpaceOnUse">'
            '<circle cx="2" cy="2" r="4" fill="#502020" opacity="0.5"/>'
            '</pattern>'
            ...
            '<stop offset="50%" stop-color="#880000" stop-opacity="0.4"/>'
            '<stop offset="100%" stop-color="#440000" stop-opacity="0.0"/>'
            '</radialGradient>'
            '</defs>'
        )
        ...
        svg.append(f'<rect x="{self.cfg.sand_zone_x}" y="0" width="{w - self.cfg.sand_zone_x}" height="{h}" fill="#0c0808"/>')
        ...
        # Structural UI
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ff4444" font-weight="bold">ECO-SIM: Ecosystem Mortality</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#bb8888">Death by Starving &amp; Old Age Limits</text>')
        ...
        # Death Events - vivid glow + pulsing ring at moment of death
        for death in self.sim.death_events:
            ...
            d_col = "#888888" if death["reason"] == "old_age" else "#ff3333"

            # Death glow halo at death position
            svg.append(f'<circle cx="{dpos[0]:.1f}" cy="{dpos[1]:.1f}" r="12" fill="url(#deathGlow)">')
            svg.append(f'<animate attributeName="opacity" values="{";".join(op_vals)}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
            svg.append(f'<animate attributeName="r" values="6;24;6" dur="0.8s" begin="{(dframe / self.cfg.frames) * dur:.2f}s" repeatCount="3"/>')
            svg.append('</circle>')

            svg.append(f'<g>')
            svg.append(f'<line x1="{dpos[0]-7:.1f}" y1="{dpos[1]-7:.1f}" x2="{dpos[0]+7:.1f}" y2="{dpos[1]+7:.1f}" stroke="{d_col}" stroke-width="3.5" stroke-linecap="round">')
        ...
        for idx in range(self.cfg.max_particles):
            ...
            svg.append(f'<circle r="{rad}" fill="{accent}">')
            svg.append(f'<animate attributeName="cx" values="{path_x}" dur="{dur}s" repeatCount="indefinite"/>')
            svg.append(f'<animate attributeName="cy" values="{path_y}" dur="{dur}s" repeatCount="indefinite"/>')
            svg.append(f'<animate attributeName="opacity" values="{op_str}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
...
def main():
    ...
    save_svg_to_drive(svg_content, 'notebook_22')
