...
# MODULE 29: Alarm Propagation (Flee Vector Wave)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Alarm Propagation (Flee Vector Wave) emphasizing community stewardship nodes.
- Indicator species: Ipe-amarelo (Handroanthus ochraceus).
...
class SimulationConfig:
    ...
    frames: int = 480          # Prompt 29: 480 frames
    fps:    int = 10
    ...
    # NEW: Alarm Propagation Mechanics
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        self.alarm_history              = [] # List of numpy arrays storing alarm level
    ...
    def step(self, frame_idx: int):
        ...
        if hungry_predators.any() and prey_mask.any():
            ...
            if chase.any():
                ...
                # Prey senses the chase and alarms!
                prey_hunted_idx = prey_idx[target_idx[chase]]
                self.alarm_level[prey_hunted_idx] = 1.0
                # Vector is away from predator
                ...
                if capture.any():
                    ...
                    for i in range(len(captured_hunters)):
                        h_id = captured_hunters[i]; p_id = captured_prey[i]
        ...
        if frame_idx >= self.cfg.fire_start_frame:
            ...
            if len(self.fire_nodes) > 0:
                ...
                if flee_mask.any():
                    ...
                    # Fire also creates direct alarm
                    self.alarm_level[flee_mask] = 1.0
                    self.alarm_vectors[flee_mask] = fv / mn_fire[flee_mask].unsqueeze(1).clamp(min=1.0)

        self.fire_nodes_history.append(self.fire_nodes.cpu().numpy().copy())

        # ── NEW: ALARM PROPAGATION (Wave mechanics) ───────────────────────────
        # Alarmed particles broadcast alarm to non-alarmed neighbors
        alarm_repulsion = torch.zeros_like(self.vel)
        is_alarmed = (self.alarm_level > 0.5) & surface_mask
        not_alarmed = (self.alarm_level <= 0.5) & surface_mask

        if is_alarmed.any() and not_alarmed.any():
            alarm_pos = self.pos[is_alarmed]
            quiet_pos = self.pos[not_alarmed]

            d_al = torch.cdist(quiet_pos, alarm_pos)
            min_dal, target_al_idx = torch.min(d_al, dim=1)

            infected = min_dal < self.cfg.alarm_propagation_radius
            if infected.any():
                infected_global_idx = not_alarmed.nonzero().squeeze(1)[infected]
                source_global_idx = is_alarmed.nonzero().squeeze(1)[target_al_idx[infected]]

                # Copy the alarm state
                self.alarm_level[infected_global_idx] = 1.0

                # Blend the flee vector (copy from source + slightly away from source)
                # This causes the fleeing cascade wave effect
                source_vecs = self.alarm_vectors[source_global_idx]
                away_from_source = self.pos[infected_global_idx] - self.pos[source_global_idx]
                away_from_source = away_from_source / (min_dal[infected].unsqueeze(1).clamp(min=1.0))

                combo_vec = (source_vecs + away_from_source) / 2.0
                combo_vec = combo_vec / combo_vec.norm(dim=1, keepdim=True).clamp(min=1e-5)
                self.alarm_vectors[infected_global_idx] = combo_vec

                # Create visual event marks for UI
                for idx in infected_global_idx:
                    if random.random() < 0.2: # Sample to avoid overloading visuals
                        self.alarm_events.append({"pos": self.pos[idx].cpu().numpy().copy(), "frame": frame_idx})

        # Apply alarm repulsion to everyone alarmed
        act_alarmed = (self.alarm_level > 0.1) & surface_mask & ~flee_mask & ~self.is_carnivore
        if act_alarmed.any():
            alarm_repulsion[act_alarmed] = self.alarm_vectors[act_alarmed] * self.cfg.alarm_panic_force * self.alarm_level[act_alarmed].unsqueeze(1)
        ...
        if frame_idx % self.cfg.swarm_spawn_interval == 0:
            ...
            centre = torch.tensor([cx, cy], device=self.dev); drift = (torch.rand(2, device=self.dev) - 0.5) * self.cfg.swarm_drift * 2.0
        ...
        for sw in self.swarms:
            ...
            chase = (d_sw < self.cfg.insect_chase_radius) & self.is_insectivore & surface_mask & ~flee_mask & (self.alarm_level < 0.5)
        ...
        if self.carrion_sites and self.is_scavenger.any():
            ...
            if scav_surf.any():
                ...
                if near.any():
                    ...
                    scav_attraction[si[near]] = tangent; self.energy[si[near]] += self.cfg.scavenger_energy_gain
        ...
        homeless_mask = active_mask & ~has_nest_mask & (self.alarm_level < 0.5)
        nested_mask   = active_mask & has_nest_mask
        if nested_mask.any():
            self.energy[nested_mask] += self.cfg.nest_energy_bonus
            nest_idxs = self.particle_nest[nested_mask]; nest_positions = self.nest_nodes[nest_idxs]
        ...
        if current_pop < self.cfg.carrying_capacity:
            ready = surface_mask & ~flee_mask & (self.energy > self.cfg.mating_energy_threshold) & (self.alarm_level < 0.5)
            if ready.any():
        ...
        if surface_mask.any():
            ...
            # Agents carrying high alarm level turn faster (panic agility)
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        # Title
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ffffff" font-weight="bold">ECO-SIM: Alarm Propagation Wave</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#b0bec5">Fear vectors spread socially across agents</text>')
        ...
        # Alarm Pulse Events (Visualization highlights where alarm infection triggers)
        for ae in self.sim.alarm_events:
            apos, af = ae["pos"], ae["frame"]
            op_vals = ["0.8" if af <= fi < af+5 else "0.0" for fi in range(frames)]
            r_vals = [f"{(fi-af)*4+5:.1f}" if af <= fi < af+5 else "1.0" for fi in range(frames)]
            svg.append(f'<circle cx="{apos[0]:.1f}" cy="{apos[1]:.1f}" fill="none" stroke="#ffffff" stroke-width="1.5">'
                       f'<animate attributeName="r" values="{";".join(r_vals)}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>'
        ...
        for idx in range(self.cfg.max_particles):
            ...
            for fi in range(frames):
                ...
                al_level = self.sim.alarm_history[fi][idx]

                op_vals.append("1.0" if (act and vis) else "0.0")
                # Flashes white if highly alarmed
                if act and vis and al_level > 0.5 and (fi % 2 == 0):
                    alarm_fills.append("#ffffff")
                else:
                    alarm_fills.append(p_col)
            ...
            if idx % 8 == 0 or idx >= self.cfg.initial_particles:
                ...
                for chunk in chunks:
                    svg.append(f'<path d="M {" L ".join(f"{p[0]:.1f},{p[1]:.1f}" for p in chunk)}" fill="none" stroke="{p_col}" stroke-opacity="0.30" stroke-width="1.5"/>')

            svg.append(f'<circle r="4" fill="{p_col}"><animate attributeName="cx" values=\"{px}\" dur="{dur}s" repeatCount="indefinite"/><animate attributeName="cy" values=\"{py}\" dur="{dur}s" repeatCount="indefinite"/><animate attributeName="opacity" values=\"{";".join(op_vals)}\" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/><animate attributeName="fill" values=\"{";".join(alarm_fills)}\" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/></circle>')
        ...
        svg.append(f'<g transform="translate({w-cw-20},340)">'
                   ...
                   f'<text x="15" y="25" fill="#cccccc" font-size="15" font-weight="bold">Feature: Alarm Propagation</text>'
                   f'<text font-weight="bold" x="15" y="48" fill="#cccccc" font-size="15">When agents flee threats (Predation or Fire),</text>'
                   f'<text font-weight="bold" x="15" y="68" fill="#cccccc" font-size="15">an "alarm state" is triggered.</text>'
                   f'<text font-weight="bold" x="15" y="88" fill="#cccccc" font-size="15">Alarm broadcasts within a {self.cfg.alarm_propagation_radius} radius,</text>'
                   f'<text font-weight="bold" x="15" y="108" fill="#cccccc" font-size="15">infecting neighbours to scatter via rapid wave.</text>'
                   f'<text font-weight="bold" x="15" y="128" fill="#cccccc" font-size="15">Alarmed particles flash white and gain +80% speed.</text>'
                   f'</g>')
...
def main():
    ...
    print(f"Initializing Alarm Wave simulator on {CONFIG.device}...")
    sim = TerraRoncaEcosystem(CONFIG)
    for fi in range(CONFIG.frames):
        sim.step(fi)
    print(f"Simulation complete. Alarm events: {len(sim.alarm_events)}. Generating SVG...")
    renderer    = EcosystemRenderer(CONFIG, sim)
    svg_content = renderer.generate_svg()
    display(HTML(svg_content))
    save_svg_to_drive(svg_content, 'notebook_29')
