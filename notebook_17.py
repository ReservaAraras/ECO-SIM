...
# MODULE 17: Seed Dispersal (Ornithochory)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Seed Dispersal (Ornithochory) emphasizing herbivory pressure gradients.
- Indicator species: Tucano-toco (Ramphastos toco).
...
class SimulationConfig:
    ...
    frames: int = 360 # Prompt requirement: 360 frames
    fps: int = 10
    ...
    # NEW: Seed Dispersal Config
    seed_carry_duration: float = 60.0  # Frames to hold a seed before dropping
    seed_pickup_chance: float = 0.05   # Chance per frame in canopy to grab a seed
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        self.dropped_seeds = [] # Dynamic array to hold static seed XY coords over time
        ...
        self.carrying_seed_history = [] # For SMIL animation rendering
    ...
    def step(self, frame_idx: int):
        ...
        # -------------------------------------------------------------------------
        # NEW: SEED DISPERSAL LOGIC (Ornithochory)
        # -------------------------------------------------------------------------
        # 1. Picking up seeds while eating
        can_grab_mask = feeding_mask & ~self.has_seed # Must be feeding and not already holding a seed
        if can_grab_mask.any():
            ...
            if successful_grabs.any():
                ...
                # Add tiny randomness so they don't all drop seeds on exactly the same frame
                jitter = torch.randn(self.cfg.num_particles, device=self.dev) * 5.0
                self.seed_timers[successful_grabs] = jitter[successful_grabs]

        # 2. Advance timers for those carrying seeds
        self.seed_timers[self.has_seed] += 1.0

        # 3. Dropping seeds
        # Combine base duration with species-specific modifiers if needed, but we'll use base + random jitter
        ...
        if drop_mask.any():
            ...
            for p in self.pos[drop_mask]:
                # Record dropped seed location & the frame it appeared
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        svg = [f'<svg viewBox="0 0 {w} {h}" xmlns="http://www.w3.org/2000/svg" style="background-color:#050c05; font-family:system-ui, -apple-system, sans-serif;">']

        svg.append(
            '<defs>'
            '<pattern id="dotGrid" width="40" height="40" patternUnits="userSpaceOnUse">'
            '<circle cx="2" cy="2" r="4" fill="#2a6040" opacity="0.5"/>'
            '</pattern>'
            ...
            '<stop offset="0%" stop-color="#ff80c0" stop-opacity="0.85"/>'
            '<stop offset="60%" stop-color="#fe4db7" stop-opacity="0.3"/>'
            '<stop offset="100%" stop-color="#fe4db7" stop-opacity="0.0"/>'
            '</radialGradient>'
            '<radialGradient id="seedGold" cx="50%" cy="50%" r="50%">'
            '<stop offset="0%" stop-color="#fff380" stop-opacity="0.9"/>'
            '<stop offset="100%" stop-color="#ffaa00" stop-opacity="0.0"/>'
            '</radialGradient>'
            '</defs>'
        )

        svg.append(f'<rect width="{w}" height="{h}" fill="#050c05"/>')
        ...
        # Fruiting Nodes - vivid with petal-like pulsing arcs
        ...
        for i, fn in enumerate(f_nodes):
            # Outer ambient bloom glow
            svg.append(f'<circle cx="{fn[0]}" cy="{fn[1]}" fill="url(#fruitGlow)">')
            svg.append(f'<animate attributeName="r" values="{";".join([f"{35 + 45*f:.1f}" for f in self.sim.fruiting_intensity_history])}" dur="{dur}s" repeatCount="indefinite"/>')
            svg.append(f'<animate attributeName="opacity" values="{f_op_vals}" dur="{dur}s" repeatCount="indefinite"/>')
            svg.append('</circle>')

            # Main fruiting circle (animates r + fill)
            ...
            # Petal-like rings (3 ringing dashes at different radii, rotating phase)
            for petal in range(3):
                poff = petal * 0.7
                svg.append(
                    f'<circle cx="{fn[0]}" cy="{fn[1]}" r="18" fill="none" stroke="#ff80c0" stroke-width="2.5" stroke-dasharray="4 12">'
                    f'<animate attributeName="r" values="12;28;12" dur="2.4s" begin="{poff}s" repeatCount="indefinite"/>'
                    f'<animate attributeName="stroke-opacity" values="0.2;0.9;0.2" dur="2.4s" begin="{poff}s" repeatCount="indefinite"/>'
                    f'</circle>'
                )
        ...
        # Structural UI
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ff80c0" font-weight="bold">ECO-SIM: Ornithochory</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#88aa88">Frugivores Carrying &amp; Dispersing Seeds</text>')

        # Render STATIC Dropped Seeds with sparkle effect
        for seed_data in self.sim.dropped_seeds:
             ...
             for f_idx in range(self.cfg.frames):
                  op_vals.append("1.0" if f_idx >= s_frame else "0.0")
             s_op_str = ";".join(op_vals)

             # Gold seed glow
             svg.append(f'<circle cx="{spos[0]:.1f}" cy="{spos[1]:.1f}" r="10" fill="url(#seedGold)">')
             svg.append(f'<animate attributeName="opacity" values="{s_op_str}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
             svg.append('</circle>')

             # Seed center dot
             svg.append(f'<circle cx="{spos[0]:.1f}" cy="{spos[1]:.1f}" r="3.5" fill="#ffe43f" stroke="#ffaa00" stroke-width="1">')
             svg.append(f'<animate attributeName="opacity" values="{s_op_str}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
             svg.append('</circle>')

             # Sparkle ring at drop moment
             burst_begin = f"{(s_frame / self.cfg.frames) * dur:.2f}s"
             svg.append(
                 f'<circle cx="{spos[0]:.1f}" cy="{spos[1]:.1f}" r="4" fill="none" stroke="#fff380" stroke-width="2">'
                 f'<animate attributeName="r" values="3;18;3" dur="1.2s" begin="{burst_begin}" repeatCount="indefinite"/>'
                 f'<animate attributeName="stroke-opacity" values="0.9;0.0;0.9" dur="1.2s" begin="{burst_begin}" repeatCount="indefinite"/>'
                 f'</circle>'
             )
        ...
        for idx in range(self.cfg.num_particles):
            ...
            if idx % 6 == 0 or is_frugivore:
                 ...
                 s_w = 3.0 if is_frugivore else 1.5
                 t_op = 0.50 if is_frugivore else 0.25
            ...
            r_pulse = "4.5;7.5;4.5" if is_frugivore else "3.0;5.0;3.0"

            # Glow halo for frugivores + occasional other birds
            if is_frugivore or idx % 6 == 0:
                halo_r = "9;16;9" if is_frugivore else "6;11;6"
            ...
            # Carried-seed indicator (bright gold token)
            if is_frugivore:
                 svg.append(f'<circle r="4" fill="#ffe43f" filter="url(#glow)">')
                 svg.append(f'<animate attributeName="cx" values="{path_x}" dur="{dur}s" repeatCount="indefinite"/>')
                 svg.append(f'<animate attributeName="cy" values="{path_y}" dur="{dur}s" repeatCount="indefinite"/>')
                 svg.append(f'<animate attributeName="opacity" values="{seed_op_str}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
                 svg.append(f'<animate attributeName="r" values="2;5;2" dur="0.9s" begin="{r_begin}" repeatCount="indefinite"/>')
                 svg.append('</circle>')
...
def main():
    ...
    print(f"Initializing Seed Dispersal simulator on {CONFIG.device}...")
    ...
    save_svg_to_drive(svg_content, 'notebook_17')
