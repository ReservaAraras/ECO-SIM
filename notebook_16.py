...
# MODULE 16: Fruiting Phenology (Resource Node Pulsing)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Fruiting Phenology (Resource Node Pulsing) emphasizing predator-prey cascade timing.
- Indicator species: Tuiuiu (Jabiru mycteria).
...
class SimulationConfig:
    ...
    frames: int = 350 # Prompt requirement: 350 frames
    fps: int = 10
    ...
    # NEW: Fruiting Phenology Config
    fruiting_cycle_frames: float = 150.0  # Time for a full dormacy/fruiting cycle
    fruiting_radius: float = 50.0 # Pull distance when fruiting
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        # Track frugivores for specifically strong attraction to fruiting nodes
        ...
        # NEW Fruiting Nodes (Giant Pequi or Jatobá trees that pulse)
        ...
        # Track fruiting intensity for rendering the blooming VFX
    ...
    def step(self, frame_idx: int):
        ...
        # -------------------------------------------------------------------------
        # NEW: FRUITING PHENOLOGY (Trees Pulse with ripe resources periodically)
        # -------------------------------------------------------------------------
        fruit_cycle = (frame_idx % self.cfg.fruiting_cycle_frames) / self.cfg.fruiting_cycle_frames
        # Math: Sinusoid but heavily clamped at 0 to mimic "bursts" of fruiting instead of smooth sine wave
        raw_fruit_val = math.sin(fruit_cycle * 2 * math.pi)
        fruiting_intensity = max(0.0, raw_fruit_val) # Ranges 0.0 to 1.0, 0 for half the cycle
        ...
        if fruiting_intensity > 0.0:
            # Strong pull that scales with the bloom intensity! Only strong when fully ripe.
            f_pull_vectors = self.fruiting_nodes[closest_fruits] - self.pos
            # It heavily specifically attracts Frugivores within a 200 unit smell radius!
            smell_mask = (min_dist_fruit < 200.0) & self.is_frugivore
            if smell_mask.any():
                ...
                # Slow down frugivores foraging actively in the canopy
                foraging_mask = smell_mask & (min_dist_fruit < self.cfg.fruiting_radius)
                if foraging_mask.any():
                    # Random chaotic bouncing inside the tree canopy
        ...
        if surface_mask.any():
             base_forces = (self.vel[surface_mask] * self.drags[surface_mask] +
                          ...
                          fruiting_attraction[surface_mask] + # NEW force applied
        ...
        # Super slow feeding in canopy
        feeding_mask = (min_dist_fruit < self.cfg.fruiting_radius) & self.is_frugivore & surface_mask & (fruiting_intensity > 0.1)
        if feeding_mask.any():
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        # NEW: Fruiting Phenology Nodes (Pulsing visually from gray dormant to giant blooming pink/magenta)
        ...
        for i, fn in enumerate(f_nodes):
            ...
            svg.append(f'<text x="{fn[0]+22}" y="{fn[1]+5}" font-size="15" fill="#cccccc" font-weight="bold">Pequizeiro Fruit Node {i+1}</text>')

        # Structural UI
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ffffff" font-weight="bold">ECO-SIM: Fruiting Phenology</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#b0bec5">Resource Pulsing (Dormancy vs Blooming)</text>')
...
def main():
    ...
    print(f"Initializing Fruiting Phenology simulator on {CONFIG.device}...")
    ...
    save_svg_to_drive(svg_content, 'notebook_16')
