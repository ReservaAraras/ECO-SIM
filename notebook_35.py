# -*- coding: utf-8 -*-
# MODULE 35: Noise Zones (Circular repulsion - tourist vehicles)
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Noise Zones (Circular repulsion - tourist vehicles) emphasizing integrated landscape coupling.
- Indicator species: Orquidea-cerrado (Cyrtopodium sp.).
...
BIODIVERSITY_DB = {
    ...
    "Gado-Nelore":      {"speed":1.2,"color":"#8d6e63","weight":0.15,"diet":"Grazer",     "cohesion":0.35,"drag":0.80,"max_turn":0.02,"lifespan_base":800,"flies":False},
}
...
class SimulationConfig:
    ...
    width:int=1280; height: int = CANVAS_HEIGHT; frames:int=540; fps:int=10
    ...
    logging_start_frame:int=100; logging_interval:int=45; logging_disturbance_radius:float=80.0
    num_nest_nodes:int=10; nest_energy_bonus:float=2.0; homeless_restless_force:float=2.5
    ...
    # NEW: Noise Zones
    num_noise_vehicles:int=4
    noise_radius:float=90.0
    noise_repulsion_force:float=18.0
    noise_speed:float=2.5
    noise_alarm_trigger:bool=True
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg):
        ...
        # Nest / Roost nodes
        nx=[120.0,280.0,450.0,600.0,750.0,900.0,1050.0,200.0,500.0,1100.0]
        ny=[150.0,300.0,100.0,450.0,200.0,350.0,120.0,480.0,320.0,400.0]
        self.nest_nodes=torch.tensor(list(zip(nx,ny)),device=self.dev,dtype=torch.float32)
        self.nest_active=[True]*cfg.roost_nodes_initial
        ...
        self.keystone_event=None; self.migrants_spawned=0; self.migrants_exited=0

        # NEW: Tourist noise vehicles - wander randomly with sinusoidal paths
        self.noise_pos=torch.rand((cfg.num_noise_vehicles,2),device=self.dev)*torch.tensor([cfg.width,cfg.height],device=self.dev)
        self.noise_vel=(torch.rand((cfg.num_noise_vehicles,2),device=self.dev)-0.5)*cfg.noise_speed*2.0
        self.noise_phase=torch.rand(cfg.num_noise_vehicles,device=self.dev)*math.pi*2.0  # for sinusoidal wobble
        ...
        self.noise_pos_history:List[np.ndarray]=[]  # track vehicle positions
    ...
    def _spawn_migrant(self,fi):
        ...
        ni=free[0]; ms="Andorinha-rio"; mid=self.guilds.index(ms)
        ...
        self.is_active[ni]=True
        y=random.uniform(self.cfg.migration_span_y_min,self.cfg.migration_span_y_max)
        ...
        self.is_underground[ni]=False; self.has_seed[ni]=False; self.courage[ni]=1.0; self.migrants_spawned+=1

    def step(self, fi):
        """Function `step` -- simulation component."""

        old_vel=self.vel.clone(); am=self.is_active.clone(); sm=am & ~self.is_underground

        # Keystone
        if fi==self.cfg.keystone_removal_frame:
            lm=am & ~self.is_migrant & ~self.is_grazer
            if lm.any():
                c=torch.bincount(self.species_id[lm]); kid=torch.argmax(c).item(); tm=(self.species_id==kid)&am
                ...
                for idx in tm.nonzero().squeeze(1):
                    ...
                    self.death_events.append({"pos":pn,"frame":fi,"color":self.colors[idx],"reason":"extinction"})
        ...
        # Logging
        if fi>=self.cfg.logging_start_frame and (fi-self.cfg.logging_start_frame)%self.cfg.logging_interval==0:
            ...
            if alive:
                ...
                dt2=torch.norm(self.pos-torch.tensor(tp,device=self.dev),dim=1)
                dist=dt2<self.cfg.logging_disturbance_radius; dist&=sm
                if dist.any():
                    self.alarm_level[dist]=1.0; away=self.pos[dist]-torch.tensor(tp,device=self.dev)
                    self.alarm_vectors[dist]=away/away.norm(dim=1,keepdim=True).clamp(min=1.0)

        # NEW: MOVE NOISE VEHICLES (sinusoidal wander)
        self.noise_phase+=0.05
        wobble=torch.stack([torch.cos(self.noise_phase),torch.sin(self.noise_phase*0.7)],dim=1)*self.cfg.noise_speed
        self.noise_vel=self.noise_vel*0.92+wobble*0.08+torch.randn_like(self.noise_vel)*0.3
        spd_n=torch.norm(self.noise_vel,dim=1,keepdim=True).clamp(min=0.1)
        self.noise_vel=(self.noise_vel/spd_n)*self.cfg.noise_speed
        self.noise_pos+=self.noise_vel
        self.noise_pos[:,0]=self.noise_pos[:,0].clamp(50,self.cfg.width-50)
        self.noise_pos[:,1]=self.noise_pos[:,1].clamp(50,self.cfg.height-50)
        # bounce off walls
        hit_x=(self.noise_pos[:,0]<=50)|(self.noise_pos[:,0]>=self.cfg.width-50)
        hit_y=(self.noise_pos[:,1]<=50)|(self.noise_pos[:,1]>=self.cfg.height-50)
        self.noise_vel[hit_x,0]*=-1; self.noise_vel[hit_y,1]*=-1
        self.noise_pos_history.append(self.noise_pos.cpu().numpy().copy())

        self.age[am]+=1.0; self.energy[am]-=self.cfg.energy_decay
        ...
        if fi>=self.cfg.migration_start_frame and random.random()<self.cfg.migration_spawn_rate: self._spawn_migrant(fi)
        am=self.is_active.clone(); sm=am & ~self.is_underground
        mig_act=am & self.is_migrant
        if mig_act.any():
        ...
        dead=(am&(self.energy<=0.0))|(am&(self.age>=self.lifespan))
        if dead.any():
            for idx in dead.nonzero().squeeze(1):
                ...
                if n>=0: self.nest_occupant[n]=-1; self.particle_nest[idx]=-1
                pn=self.pos[idx].cpu().numpy().copy()
                self.death_events.append({"pos":pn,"frame":fi,"color":self.colors[idx],"reason":"old_age" if self.age[idx]>=self.lifespan[idx] else "starvation"})
                self.carrion_sites.append({"pos":pn,"spawn_frame":fi,"expire_frame":fi+self.cfg.carrion_linger_frames})
        am=self.is_active.clone(); sm=am & ~self.is_underground
        pop=int(am.sum().item())
        ...
        if sm.any():
            ...
            if hn.any():
                sp2=torch.matmul(ir.float(),self.pos[sm]); ap=sp2[hn]/nc[hn].unsqueeze(1).float()
                pull=ap-self.pos[sm][hn]; pm=self.cohesions[sidx[hn]].unsqueeze(1)*self.cfg.flock_cohesion_multiplier
                fa2[sidx[hn]]=(pull/pull.norm(dim=1,keepdim=True).clamp(min=1.0))*pm
            self.cohesion_stats.append(nc.float().mean().item())
        else: self.cohesion_stats.append(0.0)

        # NEW: NOISE ZONE REPULSION
        noise_rep=torch.zeros_like(self.vel)
        for vi in range(self.cfg.num_noise_vehicles):
            vp=self.noise_pos[vi]
            d_noise=torch.norm(self.pos-vp,dim=1)
            affected=sm & (d_noise<self.cfg.noise_radius)
            if affected.any():
                away=self.pos[affected]-vp; mag=(1.0-d_noise[affected]/self.cfg.noise_radius)**2
                noise_rep[affected]+=(away/d_noise[affected].unsqueeze(1).clamp(min=1.0))*mag.unsqueeze(1)*self.cfg.noise_repulsion_force
                if self.cfg.noise_alarm_trigger:
                    self.alarm_level[affected]=torch.clamp(self.alarm_level[affected]+0.3,max=1.0)
                    anorm=away/d_noise[affected].unsqueeze(1).clamp(min=1.0)
                    self.alarm_vectors[affected]=anorm

        # Road+Fence
        road_rep=torch.zeros_like(self.vel); dtr=torch.abs(self.pos[:,0]-self.cfg.road_x_center)
        sc=sm&(dtr<self.cfg.road_fear_radius)&(self.courage<(1.0-self.cfg.road_crossing_courage))&~self.is_grazer
        if sc.any(): road_rep[sc,0]=torch.sign(self.pos[sc,0]-self.cfg.road_x_center)*(1.0-(dtr[sc]/self.cfg.road_fear_radius))**2*self.cfg.road_fear_force_base
        fence_rep=torch.zeros_like(self.vel); gm=sm&~self.flies
        if gm.any():
            for fy in self.cfg.fence_y_positions:
                dy=torch.abs(self.pos[:,1]-fy); nf=gm&(dy<self.cfg.fence_fear_radius)
                if nf.any(): fence_rep[nf,1]+=torch.sign(self.pos[nf,1]-fy)*(1.0-dy[nf]/self.cfg.fence_fear_radius)**2*self.cfg.fence_repulsion_force

        # Hunting
        hunt=torch.zeros_like(self.vel)
        pm2=sm&self.is_carnivore; qm=sm&(self.is_frugivore|self.is_insectivore|self.is_migrant); hg=pm2&(self.satiation_timers<=0)
        if hg.any() and qm.any():
            pi2=hg.nonzero().squeeze(1); qi=qm.nonzero().squeeze(1)
            dh=torch.cdist(self.pos[hg],self.pos[qm]); md,ti=torch.min(dh,dim=1); chase=md<self.cfg.hunting_chase_radius
            if chase.any():
                tgt=self.pos[qi[ti[chase]]]; hnt=self.pos[pi2[chase]]; pll=tgt-hnt
                hunt[pi2[chase]]=(pll/md[chase].unsqueeze(1).clamp(min=1.0))*self.cfg.hunting_chase_force
        ...
        karst=(self.nectar_nodes[cl_nec]-self.pos)/mn_nec.unsqueeze(1).clamp(min=1.0)
        cycp=(fi%self.cfg.vereda_cycle_frames)/self.cfg.vereda_cycle_frames
        ...
        vera=(self.vereda_nodes[cl_ver]-self.pos)/mn_ver.unsqueeze(1).clamp(min=1.0)*(self.cfg.vereda_base_attraction*wlvl*hmod).unsqueeze(1)
        flooded=mn_ver<cvrad; vera[flooded]*=0.1
        if (flooded&sm&~self.is_grazer).any(): self.energy[flooded&sm&~self.is_grazer]+=self.cfg.energy_gain_vereda*hmod[flooded&sm&~self.is_grazer]
        gz=sm&self.is_grazer
        if gz.any():
            pv=self.vereda_nodes[cl_ver[gz]]-self.pos[gz]; vera[gz]+=(pv/mn_ver[gz].unsqueeze(1).clamp(min=1.0))*self.cfg.cattle_chase_vereda_force
            eat=gz&(mn_ver<self.cfg.cattle_graze_radius)
            if eat.any():
                self.energy[eat]+=self.cfg.cattle_graze_gain
                for ni in cl_ver[eat]: self.vereda_health[ni]-=self.cfg.cattle_resource_depletion_rate
                self.vereda_health.clamp_(0.0,1.0)
        fic=(fi%self.cfg.fruiting_cycle_frames)/self.cfg.fruiting_cycle_frames
        ...
        frut=torch.zeros_like(self.vel); feed=torch.zeros(self.cfg.max_particles,device=self.dev,dtype=torch.bool)
        if fi_int>0.0:
            sml=(mn_fr<200.0)&self.is_frugivore&sm
            if sml.any():
                frut[sml]=(self.fruiting_nodes[cl_fr[sml]]-self.pos[sml])/mn_fr[sml].unsqueeze(1).clamp(min=1.0)*(self.cfg.fruiting_base_attraction*fi_int)
                fr2=sml&(mn_fr<self.cfg.fruiting_radius); feed=fr2&(fi_int>0.1)
                if fr2.any(): frut[fr2]+=torch.randn_like(frut[fr2])*2.0; self.energy[fr2]+=self.cfg.energy_gain_fruiting*fi_int

        # Seeds
        if feed.any():
            ok=feed&~self.has_seed&(torch.rand(self.cfg.max_particles,device=self.dev)<self.cfg.seed_pickup_chance)
            if ok.any(): self.has_seed[ok]=True; self.seed_timers[ok]=0.0
        self.seed_timers[self.has_seed]+=1.0
        drop=self.has_seed&(self.seed_timers>=self.cfg.seed_carry_duration)&sm
        if drop.any():
            self.has_seed[drop]=False
            for p in self.pos[drop]: self.dropped_seeds.append({"pos":p.cpu().numpy().copy(),"frame":fi})

        # Fire
        fire_rep=torch.zeros_like(self.vel); flee=torch.zeros(self.cfg.max_particles,device=self.dev,dtype=torch.bool)
        if fi>=self.cfg.fire_start_frame:
            if len(self.fire_nodes)==0: self.fire_nodes=torch.tensor([[1000.0,300.0]],device=self.dev)
            elif fi%2==0 and len(self.fire_nodes)<self.cfg.max_fire_nodes:
            ...
            if len(self.fire_nodes)>0:
                df=torch.cdist(self.pos,self.fire_nodes); mn_f,cl_f=torch.min(df,dim=1); flee=(mn_f<self.cfg.fire_flee_radius)&sm
                if flee.any():
                    fv=self.pos[flee]-self.fire_nodes[cl_f[flee]]
                    fire_rep[flee]=(fv/mn_f[flee].unsqueeze(1).clamp(min=1.0)**1.5)*self.cfg.fire_flee_force*30.0
                    self.alarm_level[flee]=1.0; self.alarm_vectors[flee]=fv/mn_f[flee].unsqueeze(1).clamp(min=1.0)
        self.fire_nodes_history.append(self.fire_nodes.cpu().numpy().copy())

        # Alarm wave
        al_rep=torch.zeros_like(self.vel)
        ial=(self.alarm_level>0.5)&sm; nal=(self.alarm_level<=0.5)&sm
        if ial.any() and nal.any():
            ...
            if inf.any():
                ...
                for idx in ig:
                    if random.random()<0.15: self.alarm_events.append({"pos":self.pos[idx].cpu().numpy().copy(),"frame":fi})
        aal=(self.alarm_level>0.1)&sm&~flee&~self.is_carnivore
        if aal.any(): al_rep[aal]=self.alarm_vectors[aal]*self.cfg.alarm_panic_force*self.alarm_level[aal].unsqueeze(1)

        # Swarms
        if fi%self.cfg.swarm_spawn_interval==0:
            cx,cy=random.uniform(100,self.cfg.width-100),random.uniform(80,self.cfg.height-80)
            self.swarms.append({"centre":torch.tensor([cx,cy],device=self.dev),"drift":(torch.rand(2,device=self.dev)-0.5)*self.cfg.swarm_drift*2.0,"spawn_frame":fi,"die_frame":fi+self.cfg.swarm_lifetime})
        insect=torch.zeros_like(self.vel); lsw=[]
        for sw in self.swarms:
            if fi>=sw["die_frame"]: continue
            sw["centre"]+=sw["drift"]; sw["centre"]=sw["centre"].clamp(torch.zeros(2,device=self.dev),torch.tensor([self.cfg.width,self.cfg.height],device=self.dev)); lsw.append(sw)
            dsw=torch.norm(self.pos-sw["centre"],dim=1)
            ch3=(dsw<self.cfg.insect_chase_radius)&self.is_insectivore&sm&~flee&(self.alarm_level<0.5)
            if ch3.any(): insect[ch3]+=(sw["centre"]-self.pos[ch3])/dsw[ch3].unsqueeze(1).clamp(min=1.0)*self.cfg.insect_chase_force
            fsw=(dsw<self.cfg.swarm_radius)&self.is_insectivore&sm
            if fsw.any(): self.energy[fsw]+=self.cfg.insect_energy_gain
        self.swarms=lsw

        # Scavenging
        scav=torch.zeros_like(self.vel)
        self.carrion_sites=[c for c in self.carrion_sites if c["expire_frame"]>fi]
        if self.carrion_sites and self.is_scavenger.any():
            ...
            dc=torch.cdist(self.pos,cp3); mc,cc=torch.min(dc,dim=1); ss2=self.is_scavenger&sm&~flee
            if ss2.any():
                cl2=cp3[cc[ss2]]; ds2=mc[ss2]; si2=ss2.nonzero().squeeze(1); farp=ds2>self.cfg.carrion_circle_radius
                if farp.any(): scav[si2[farp]]=(cl2[farp]-self.pos[si2[farp]])/ds2[farp].unsqueeze(1).clamp(min=1.0)*6.0
                np2=~farp
                if np2.any():
                    tc2=self.pos[si2[np2]]-cl2[np2]; tg=torch.stack([-tc2[:,1],tc2[:,0]],dim=1)
                    scav[si2[np2]]=tg/tg.norm(dim=1,keepdim=True).clamp(min=1e-5)*5.0; self.energy[si2[np2]]+=self.cfg.scavenger_energy_gain
        ...
        for ni in range(self.cfg.roost_nodes_initial):
            ...
            if not self.nest_active[ni] and self.nest_occupant[ni]>=0: self.nest_occupant[self.nest_occupant[ni]]=-1; self.particle_nest[self.nest_occupant[ni]]=-1; self.nest_occupant[ni]=-1
        hm=self.particle_nest>=0; homeless=am&~hm&(self.alarm_level<0.5)&~self.is_migrant&~self.is_grazer
        nested=am&hm
        if nested.any():
            self.energy[nested]+=self.cfg.roost_energy_bonus; ni2=self.particle_nest[nested]; np3=self.nest_nodes[ni2]
            d_n=torch.norm(self.pos[nested]-np3,dim=1); nest[nested]=(np3-self.pos[nested])/d_n.unsqueeze(1).clamp(min=1.0)*self.cfg.roost_attract_force
        ...
        if free_nests and homeless.any():
            ...
            mf2,bf2=torch.min(df2,dim=1); hli=homeless.nonzero().squeeze(1); ce=mf2<self.cfg.roost_attract_radius
            if ce.any():
                tgt2=fnp[bf2[ce]]; dt3=mf2[ce]; nest[hli[ce]]=(tgt2-self.pos[hli[ce]])/dt3.unsqueeze(1).clamp(min=1.0)*self.cfg.roost_attract_force
                arr=dt3<20.0
                if arr.any():
        ...
        if homeless.any(): nest[homeless]+=torch.randn(int(homeless.sum()),2,device=self.dev)*self.cfg.homeless_restless_force
        ...
        # Reproduction
        if pop<self.cfg.carrying_capacity:
            rdy=sm&~flee&(self.energy>self.cfg.mating_energy_threshold)&(self.alarm_level<0.5)&~self.is_migrant
            if rdy.any():
                for i in rdy.nonzero().squeeze(1):
                    ...
                    for j in ai2[(da3<self.cfg.mating_radius)&(da3>0.1)]:
                        if self.energy[j]>self.cfg.mating_energy_threshold and self.is_male[i]!=self.is_male[j] and self.species_id[i]==self.species_id[j] and not flee[j]:
                            ...
                            break
                    if pop>=self.cfg.carrying_capacity: break
        ...
        if sm.any():
            raw[sm]=(self.vel[sm]*self.drags[sm]+karst[sm]+vera[sm]+frut[sm]+fire_rep[sm]+insect[sm]+scav[sm]
                     +nest[sm]+fa2[sm]+hunt[sm]+al_rep[sm]+mig_attr[sm]+road_rep[sm]+fence_rep[sm]+noise_rep[sm]
                     +torch.randn_like(self.vel[sm])*0.5)
        na=torch.atan2(raw[:,1],raw[:,0]); oa=torch.atan2(old_vel[:,1],old_vel[:,0]); fa_ang=oa.clone()
        if sm.any():
            dt_turn=self.max_turns[sm].clone()
            ip=(self.alarm_level>0.5)&sm
            if ip.any(): dt_turn[ip[sm]]*=2.5
            if flee.any(): dt_turn[flee[sm]]*=3.0
            mg2=sm&self.is_migrant
            if mg2.any(): dt_turn[mg2[sm]]*=0.1
            diff=((na-oa+math.pi)%(2*math.pi)-math.pi)[sm]; fa_ang[sm]=oa[sm]+torch.clamp(diff,-dt_turn,dt_turn)
        spd=torch.norm(raw,dim=1).clamp(min=0.1); self.vel[:,0]=torch.cos(fa_ang)*spd; self.vel[:,1]=torch.sin(fa_ang)*spd
        nrm=torch.norm(self.vel,dim=1,keepdim=True).clamp(min=1e-5); ds=self.speeds.clone()
        ds[flooded&sm]*=0.7
        if feed.any(): ds[feed]*=0.4
        if ip.any(): ds[ip]*=self.cfg.alarm_panic_multiplier
        sat=(self.satiation_timers>0)&sm
        if sat.any(): ds[sat]*=self.cfg.satiation_speed_penalty
        ins=(self.pos[:,0]<self.cfg.sand_zone_x)&sm
        if ins.any(): ds[ins]*=self.cfg.sand_speed_modifier
        if flee.any(): ds[flee]*=1.5
        self.vel=(self.vel/nrm)*ds.unsqueeze(1)
        self.pos[am]+=self.vel[am]*self.cfg.dt
        wm=sm&~self.is_migrant
        if wm.any(): self.pos[wm,0]=self.pos[wm,0]%self.cfg.width; self.pos[wm,1]=self.pos[wm,1]%self.cfg.height
        self._log(pop,fi)
...
class EcosystemRenderer:
    ...
    def __init__(self,cfg,sim): self.cfg=cfg; self.sim=sim
    def generate_svg(self):
        """Function `generate_svg` -- simulation component."""

        w,h=self.cfg.width,self.cfg.height; dur=self.cfg.frames/self.cfg.fps; F=self.cfg.frames
        ...
        svg.append('<defs><radialGradient id="waterGrad"><stop offset="0%" stop-color="#00ffff" stop-opacity="0.6"/><stop offset="100%" stop-color="#0033aa" stop-opacity="0.0"/></radialGradient>'
                   '<radialGradient id="noiseGrad"><stop offset="0%" stop-color="#ffeb3b" stop-opacity="0.35"/><stop offset="100%" stop-color="#ff9800" stop-opacity="0.0"/></radialGradient>'
                   ...
                   '<pattern id="sandDot" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="5" cy="5" r="1.5" fill="#d4a373" opacity="0.15"/></pattern></defs>')
        ...
        # Vereda
        rv=";".join(f"{r:.1f}" for r in self.sim.vereda_radius_history)
        ...
        # Road + fence
        svg.append(f'<rect x="{self.cfg.road_x_center-self.cfg.road_width/2}" y="0" width="{self.cfg.road_width}" height="{h}" fill="#424242" opacity="0.75"/>')
        svg.append(f'<line x1="{self.cfg.road_x_center}" y1="0" x2="{self.cfg.road_x_center}" y2="{h}" stroke="#ffeb3b" stroke-width="1" stroke-dasharray="8" opacity="0.5"/>')
        for fy in self.cfg.fence_y_positions:
            svg.append(f'<line x1="0" y1="{fy}" x2="{w}" y2="{fy}" stroke="#a5d6a7" stroke-width="1" stroke-dasharray="5,3" opacity="0.5"/>')
        # Trees
        tn=self.sim.nest_nodes.cpu().numpy()
        for i in range(self.cfg.roost_nodes_initial):
            tx,ty=tn[i,0],tn[i,1]
            cut=next((e["frame"] for e in self.sim.logging_events if e["tree_idx"]==i),None)
            if cut is None:
                svg.append(f'<circle cx="{tx:.0f}" cy="{ty:.0f}" r="9" fill="#2e7d32" stroke="#1b5e20" stroke-width="1.5" opacity="0.85"/>')
            else:
                ao=";".join("0.85" if fi<cut else "0.0" for fi in range(F)); so=";".join("0.0" if fi<cut else "0.85" for fi in range(F))
                svg.append(f'<circle cx="{tx:.0f}" cy="{ty:.0f}" r="9" fill="#2e7d32"><animate attributeName="opacity" values="{ao}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/></circle>')
                svg.append(f'<circle cx="{tx:.0f}" cy="{ty:.0f}" r="7" fill="#5d4037" stroke="#ff7043" stroke-width="1.5"><animate attributeName="opacity" values="{so}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/></circle>')
        # NEW: Noise vehicle animation - animated circles traversing
        for vi in range(self.cfg.num_noise_vehicles):
            vx=";".join(f"{self.sim.noise_pos_history[fi][vi,0]:.1f}" for fi in range(F))
            vy=";".join(f"{self.sim.noise_pos_history[fi][vi,1]:.1f}" for fi in range(F))
            # Noise zone radius glow
            svg.append(f'<circle r="{self.cfg.noise_radius:.0f}" fill="url(#noiseGrad)" opacity="0.5">'
                       f'<animate attributeName="cx" values="{vx}" dur="{dur}s" repeatCount="indefinite"/>'
                       f'<animate attributeName="cy" values="{vy}" dur="{dur}s" repeatCount="indefinite"/>'
                       f'</circle>')
            # Vehicle dot
            svg.append(f'<circle r="8" fill="#ffeb3b" stroke="#f57f17" stroke-width="2">'
                       f'<animate attributeName="cx" values="{vx}" dur="{dur}s" repeatCount="indefinite"/>'
                       f'<animate attributeName="cy" values="{vy}" dur="{dur}s" repeatCount="indefinite"/>'
                       f'</circle>')
        # Title
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ffffff" font-weight="bold">ECO-SIM: Noise Zones (Tourist Vehicles)</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#b0bec5">Mobile repulsion circles disturb fauna</text>')
        # Particles
        for idx in range(self.cfg.max_particles):
            if not any(self.sim.active_history[fi][idx] for fi in range(F)): continue
            pc=self.sim.colors[idx]; im=self.sim.is_migrant[idx].item(); ig=self.sim.is_grazer[idx].item()
            px=";".join(f"{p[idx,0]:.1f}" for p in self.sim.trajectory_history)
            py=";".join(f"{p[idx,1]:.1f}" for p in self.sim.trajectory_history)
            op=";".join("1.0" if (self.sim.active_history[fi][idx] and self.sim.visibility_history[fi][idx]) else "0.0" for fi in range(F))
            if idx%8==0 or im or ig:
                ...
                dash='stroke-dasharray="2,2"' if im else ""; sw=1.8 if im else (3.0 if ig else 1.0); oa2=0.6 if im else (0.8 if ig else 0.3)
                for chunk in chunks:
                    svg.append(f'<path d="M {" L ".join(f"{p[0]:.1f},{p[1]:.1f}" for p in chunk)}" fill="none" stroke="{pc}" stroke-opacity="{oa2}" stroke-width="{sw}" {dash}/>')
            rad=4.5 if im else (7.0 if ig else 4.0)
        ...
        svg.append(f'<g transform="translate({w-cw-20},340)">'
                   f'<rect width="{cw}" height="{ch}" fill="#1a1a2e" rx="8" ry="8" stroke="#ffeb3b" stroke-width="1" opacity="0.95"/>'
                   f'<text x="15" y="25" fill="#cccccc" font-size="15" font-weight="bold">Feature: Noise Zones</text>'
                   f'<text font-weight="bold" x="15" y="48" fill="#cccccc" font-size="15">{self.cfg.num_noise_vehicles} tourist vehicles wander continuously.</text>'
                   f'<text font-weight="bold" x="15" y="68" fill="#cccccc" font-size="15">Each emits a {self.cfg.noise_radius:.0f} yellow repulsion radius.</text>'
                   f'<text font-weight="bold" x="15" y="88" fill="#cccccc" font-size="15">Animals inside the zone scatter outward and</text>'
                   f'<text font-weight="bold" x="15" y="108" fill="#cccccc" font-size="15">receive partial alarm states causing panic.</text>'
                   f'<text font-weight="bold" x="15" y="128" fill="#cccccc" font-size="15">Fauna habitat fragmented by ecotourism pressure.</text>'
                   f'</g>')
...
def save_svg(svg,nb):
    ...
    d=os.path.join(os.path.dirname(os.path.abspath(__file__)) if '__file__' in dir() else os.getcwd(),'svg_output')
    os.makedirs(d,exist_ok=True); fp=os.path.join(d,f'{nb}.svg')
...
def main():
    ...
    print(f"Initializing Noise Zones on {CONFIG.device}...")
    ...
    display(HTML(svg)); save_svg(svg,'notebook_35')
