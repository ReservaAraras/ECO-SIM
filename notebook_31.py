...
# MODULE 31: Road Fragmentation (Linear barrier with high fear cost)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Road Fragmentation (Linear barrier with high fear cost) emphasizing apiary influence halos.
- Indicator species: Murici (Byrsonima crassifolia).
...
class SimulationConfig:
    ...
    frames: int = 500          # Prompt 31: 500 frames
    fps:    int = 10
    ...
    # NEW: Road Fragmentation Mechanics
    road_x_center:            float = 780.0    # Fixed X coordinate for the vertical road
    road_width:               float = 20.0     # Total physical width of the road
    road_fear_radius:         float = 60.0     # Distance at which agents feel the fear gradient
    road_fear_force_base:     float = 15.0     # Base repulsion vector strength away from road
    road_crossing_courage:    float = 0.05     # Exceedingly low chance to cross the road purely randomly (1 in ~20)
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        # Give every particle a permanent "courage" factor (to bypass the road occasionally)
    ...
    def step(self, frame_idx: int):
        ...
        if frame_idx >= self.cfg.migration_start_frame:
            if random.random() < self.cfg.migration_spawn_rate:
                ...
                if len(free_slots) > 0:
                    ...
                    self.energy[new_idx] = 100.0; self.age[new_idx] = 0.0
                    self.alarm_level[new_idx] = 0.0; self.is_underground[new_idx] = False; self.has_seed[new_idx] = False

                    # Migrants have 100% courage, they fly over roads
                    self.courage[new_idx] = 1.0
        ...
        # Calculate horizontal distance to the vertical road center
        dist_to_road = torch.abs(self.pos[:, 0] - self.cfg.road_x_center)

        # Apply strict fear repulsive gradient if close to road, and lacking the courage to cross
        scared_of_road = surface_mask & (dist_to_road < self.cfg.road_fear_radius) & (self.courage < (1.0 - self.cfg.road_crossing_courage))

        if scared_of_road.any():
            # Vector pointing directly away from the road horizontally
            direction_away = torch.sign(self.pos[scared_of_road, 0] - self.cfg.road_x_center)

            # The closer you get, the exponential harder it pushes
        ...
        # Migration Flow Rightward Override
                    ...
                    for j in ai[(d_all < self.cfg.mating_radius) & (d_all > 0.1)]:
                        if (self.energy[j] > self.cfg.mating_energy_threshold and self.is_male[i] != self.is_male[j] and self.species_id[i] == self.species_id[j] and not flee_mask[j]):
                            ...
                            if len(free):
                                ...
                                self.courage[ci] = torch.rand(1, device=self.dev).item() # Offspring gets new random courage
        ...
        if surface_mask.any():
            raw[surface_mask] = (
                ...
                + road_repulsion[surface_mask]    # <--- NEW: Apply road fragmentation forces
        ...
        if surface_mask.any():
            ...
            # Agents frightened of the road turn away drastically
            if scared_of_road.any():
                dyn_t[scared_of_road[surface_mask]] *= 4.0
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        # NEW: ROAD Visualization
        # A semi-transparent asphalt-grey vertical block representing the linear barrier
        ...
        # The subtle "fear gradient" visual, representing the buffer zone agents avoid
        ...
        # Marker regions for migration flow
        ...
        # Title
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ffffff" font-weight="bold">ECO-SIM: Road Fragmentation</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#b0bec5">Ecosystem divided by impassable linear barrier</text>')
        ...
        svg.append(f'<g transform="translate({w-cw-20},340)">'
                   f'<rect width="{cw}" height="{ch}" fill="#1a1a2e" rx="8" ry="8" stroke="#ff9800" stroke-width="1" opacity="0.95"/>'
                   f'<text x="15" y="25" fill="#cccccc" font-size="15" font-weight="bold">Feature: Road Fragmentation</text>'
                   f'<text font-weight="bold" x="15" y="48" fill="#cccccc" font-size="15">A paved road spans vertically at X: {self.cfg.road_x_center}.</text>'
                   f'<text font-weight="bold" x="15" y="68" fill="#cccccc" font-size="15">Creates a {self.cfg.road_fear_radius} danger gradient.</text>'
                   f'<text font-weight="bold" x="15" y="88" fill="#cccccc" font-size="15">Almost all native animals refuse to cross, bouncing</text>'
                   f'<text font-weight="bold" x="15" y="108" fill="#cccccc" font-size="15">away dynamically, splintering the biome into two</text>'
                   f'<text font-weight="bold" x="15" y="128" fill="#cccccc" font-size="15">isolated halves. (Migrants ignore barriers via flight).</text>'
                   f'</g>')
...
def main():
    ...
    print(f"Initializing Road Fragmentation simulator on {CONFIG.device}...")
    ...
    save_svg_to_drive(svg_content, 'notebook_31')
