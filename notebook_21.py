...
# MODULE 21: Reproduction (Energy & Spawning)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Reproduction (Energy & Spawning) emphasizing roost fidelity zones.
- Indicator species: Gaviao-carijo (Rupornis magnirostris).
...
class SimulationConfig:
    ...
    frames: int = 400 # Prompt requirement: 400 frames
    fps: int = 10
    ...
    # NEW: Reproduction Dynamics
    mating_energy_threshold: float = 85.0
    mating_radius: float = 30.0
    energy_decay: float = 0.1
    energy_gain_fruiting: float = 5.0
    energy_gain_vereda: float = 2.0
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        # Spatial setup (Allocate for max_particles)
        ...
        self.energy = torch.ones(cfg.max_particles, device=self.dev) * 50.0  # Start with moderate energy
        ...
        self.birth_events = [] # For rendering spawn animations
    ...
    def step(self, frame_idx: int):
        ...
        # -------------------------------------------------------------------------
        # NEW: REPRODUCTION LOGIC (Energy thresholds & Mating)
        # -------------------------------------------------------------------------
        # Only surface animals that are well-fed can reproduce and not actively fleeing fire!
        ...
        if ready_to_mate.any():
            ...
            # Find pairwise distances among ALL active particles
            for i in ready_idx:
                # If they already mated this frame and dropped below threshold
                ...
                # Close enough, not self, opposite gender, same species, also ready
                ...
                for j in candidates:
                    if (self.energy[j] > self.cfg.mating_energy_threshold and
                        ...
                        if len(inactive_indices) > 0:
                            ...
                            self.vel[child_idx] = self.vel[i].clone() * -1.0 # burst out
                            self.energy[child_idx] = 40.0 # Starts with some energy
                            self.is_underground[child_idx] = False

                            # Deduct substantial energy from parents
                            self.energy[i] -= 45.0
                            self.energy[j] -= 45.0

                            self.birth_events.append({"pos": self.pos[child_idx].cpu().numpy().copy(), "frame": frame_idx, "color": self.colors[child_idx]})
                            break # Move to next `i` candidate

        # Update masks again after potential reproduction
        active_mask = self.is_active.clone()
        surface_mask = active_mask & ~self.is_underground
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        svg = [f'<svg viewBox="0 0 {w} {h}" xmlns="http://www.w3.org/2000/svg" style="background-color:#08060e; font-family:system-ui, -apple-system, sans-serif;">']

        svg.append(
            '<defs>'
            '<pattern id="dotGrid" width="40" height="40" patternUnits="userSpaceOnUse">'
            '<circle cx="2" cy="2" r="4" fill="#3d2070" opacity="0.5"/>'
            '</pattern>'
            ...
            '<stop offset="40%" stop-color="#e040fb" stop-opacity="0.6"/>'
        ...
        svg.append(f'<rect x="{self.cfg.sand_zone_x}" y="0" width="{w - self.cfg.sand_zone_x}" height="{h}" fill="#08060e"/>')
        ...
        # Structural UI
        svg.append(f'<text x="20" y="30" font-size="15" fill="#e040fb" font-weight="bold">ECO-SIM: Ecosystem Reproduction</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#bb88cc">Energy Threshold Mating &amp; Spawning</text>')
        ...
        # Render Birth Events - vivid multi-ring burst
        for birth in self.sim.birth_events:
             ...
             for i in range(bframe, min(bframe+25, self.cfg.frames)): op_vals[i] = "1.0"
             op_str = ";".join(op_vals)

             # Central birth flash glow blob
             svg.append(f'<circle cx="{bpos[0]:.1f}" cy="{bpos[1]:.1f}" r="18" fill="url(#birthGlow)">')
             svg.append(f'<animate attributeName="opacity" values="{op_str}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
             svg.append('</circle>')

             # Three concentric expanding rings
             for ring in range(3):
                 r_vals2 = ["1.0"] * self.cfg.frames
                 ring_op = ["0.0"] * self.cfg.frames
                 for i in range(bframe, min(bframe+20, self.cfg.frames)):
                     progress = i - bframe
                     r_vals2[i] = f"{(progress + ring * 6) * 2:.1f}"
                     ring_op[i] = f"{max(0.0, 0.9 - progress * 0.04 - ring * 0.1):.2f}"
                 svg.append(f'<circle cx="{bpos[0]:.1f}" cy="{bpos[1]:.1f}" fill="none" stroke="{bcol}" stroke-width="2.5">')
                 svg.append(f'<animate attributeName="r" values="{";".join(r_vals2)}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
                 svg.append(f'<animate attributeName="stroke-opacity" values="{";".join(ring_op)}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
                 svg.append('</circle>')

        # Render Trajectories (SMIL <animate>)
        for idx in range(self.cfg.max_particles):
            # Only render if active at SOME point
            ...
            is_newborn = idx >= self.cfg.initial_particles
            birth_accent = "#e040fb" if is_newborn else p_color

            # Glow halo
            if is_frugivore or idx % 6 == 0 or is_newborn:
                halo_r = "9;16;9" if is_newborn else ("8;14;8" if is_frugivore else "5;10;5")
                svg.append(f'<circle r="10" fill="{birth_accent}" opacity="0.30">')
                ...
                svg.append(f'<animate attributeName="r" values="{halo_r}" dur="{1.2 if is_newborn else 2.0}s" begin="{r_begin}" repeatCount="indefinite"/>')
                svg.append('</circle>')

            # Entity Node with r-pulse
            svg.append(f'<circle r="{rad}" fill="{birth_accent}">')
            svg.append(f'<animate attributeName="cx" values="{path_x}" dur="{dur}s" repeatCount="indefinite"/>')
            svg.append(f'<animate attributeName="cy" values="{path_y}" dur="{dur}s" repeatCount="indefinite"/>')
            svg.append(f'<animate attributeName="opacity" values="{op_str}" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
            svg.append(f'<animate attributeName="r" values="{r_pulse}" dur="{1.4 if is_newborn else 2.2}s" begin="{r_begin}" repeatCount="indefinite"/>')
            svg.append('</circle>')
...
def main():
    ...
    print(f"Initializing Reproduction simulator on {CONFIG.device}...")
    sim = TerraRoncaEcosystem(CONFIG)
    for frame_idx in range(CONFIG.frames):
        sim.step(frame_idx)

    print(f"Simulation complete. Total new births: {len(sim.birth_events)}. Generating SVG...")
    renderer = EcosystemRenderer(CONFIG, sim)
    svg_content = renderer.generate_svg()
    display(HTML(svg_content))
    save_svg_to_drive(svg_content, 'notebook_21')
