...
# MODULE 09: Territoriality (Hummingbirds defending Nectar Patches)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Territoriality (Hummingbirds defending Nectar Patches) emphasizing termite mound networks.
- Indicator species: Veado-catingueiro (Mazama gouazoubira).
...
BIODIVERSITY_DB = {
    "Tucano-toco (Ramphastos toco)": {"speed": 4.5, "cohesion": 0.03, "color": "#fe4db7", "weight": 0.4, "diet": "Frugivore", "seed_drop_prob": 0.073, "drag": 0.85, "max_turn": 0.1, "is_territorial": False},
    "Gralha-do-campo (Cyanocorax cristatellus)": {"speed": 5.8, "cohesion": 0.01, "color": "#00ffdc", "weight": 0.3, "diet": "Omnivore", "seed_drop_prob": 0.041, "drag": 0.90, "max_turn": 0.15, "is_territorial": False},
    "Beija-flor-tesoura (Eupetomena macroura)": {"speed": 6.2, "cohesion": 0.05, "color": "#ffe43f", "weight": 0.2, "diet": "Nectarivore", "seed_drop_prob": 0.012, "drag": 0.98, "max_turn": 0.4, "is_territorial": True},
    "Gavião-carijó (Rupornis magnirostris)": {"speed": 7.0, "cohesion": 0.005, "color": "#f44336", "weight": 0.1, "diet": "Carnivore", "seed_drop_prob": 0.001, "drag": 0.95, "max_turn": 0.2, "is_territorial": False},
}
...
class SimulationConfig:
    ...
    frames: int = 280 # Prompt requirement: 280 frames
    fps: int = 10
    device: str = 'cuda' if torch.cuda.is_available() else 'cpu'
    num_particles: int = 250
    dt: float = 0.5
    territory_radius: float = 70.0 # Area a hummingbird defends
    attack_force: float = 12.0 # Force pushing intruders away
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        # Karst / Resource Hubs (Nectar nodes where Hummingbirds set up territories)
    ...
    def step(self):
        ...
        # Determine closest nectar node for every particle
        ...
        # -------------------------------------------------------------
        # TERRITORIALITY ENGINE (Hummingbirds defending nectar)
        # -------------------------------------------------------------
        ...
        # Sub-select territorial agents (Hummingbirds) and everyone else (Intruders)
        ...
        if self.is_territorial.any() and intruders_mask.any():
            ...
            # Check which defenders are ACTUALLY near a nectar patch (claiming a territory)
            defenders_dist_to_node = min_dist_to_node[self.is_territorial]
            claiming_mask = defenders_dist_to_node < self.cfg.territory_radius

            if claiming_mask.any():
                # Get only the active defenders
                active_defenders_pos = defenders_pos[claiming_mask]

                # Check distance from active defenders to ALL intruders
                dist_def_to_int = torch.cdist(active_defenders_pos, intruders_pos)

                # For each intruder, find if there is an active defender nearby
                min_dist_to_def, closest_def_idx = torch.min(dist_def_to_int, dim=0) # Operates over intruders

                # Mask of intruders caught within a defended territory
                trespassing_mask = min_dist_to_def < self.cfg.territory_radius

                if trespassing_mask.any():
                    # Identify specific intruders and their closest angry hummingbirds
                    ...
                    # Vector pointing FROM the defender TO the intruder (Repulsion/Push)
                    push_dir = trespassing_intruders - angry_defenders
                    push_dist = min_dist_to_def[trespassing_mask].unsqueeze(1).clamp(min=1.0)

                    # Force exponentially stronger the closer the intruder gets
                    ...
                    # Intruders are blasted away
                    defense_vectors[intruders_mask.nonzero(as_tuple=True)[0][trespassing_mask]] += push_vectors

                    # Mark the active hummingbirds (for visualization lightning effects)
                    # We have to map back through two masks (claiming_mask -> is_territorial)
                    # This tells us which specific particles are currently attacking:
                    ...
                    # Also, hummingbirds snap to "chase" the intruder briefly
                    chase_vectors = (-push_dir / push_dist) * (self.cfg.attack_force * 0.5)
                    defense_vectors[global_def_idx] += chase_vectors


        # Apply forces and random wander
        raw_new_vel = self.vel * self.drags + karst_attraction + defense_vectors + torch.randn_like(self.vel) * 0.5
...
class EcosystemRenderer:
    ...
    def generate_svg(self) -> str:
        ...
        # Render Nectar Nodes (Territories)
        nnodes = self.sim.nectar_nodes.cpu().numpy()
        for i, nn in enumerate(nnodes):
            svg.append(f'<circle cx="{nn[0]}" cy="{nn[1]}" r="{self.cfg.territory_radius}" fill="#ffe43f" opacity="0.1"/>')
            svg.append(f'<circle cx="{nn[0]}" cy="{nn[1]}" r="8" fill="#ffe43f"/>')
            svg.append(f'<circle cx="{nn[0]}" cy="{nn[1]}" r="14" fill="none" stroke="#ffe43f" stroke-width="2"/>')
            svg.append(f'<text font-weight="bold" x="{nn[0]+20}" y="{nn[1]-20}" font-size="15" fill="#cccccc">Nectar Source/Territory {i+1}</text>')

        # Structural UI
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ffffff" font-weight="bold">ECO-SIM: Avian Territoriality</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#b0bec5">Hummingbird Aggressive Defense Dynamics</text>')
        ...
        for idx in range(self.cfg.num_particles):
            ...
            # Animate radius/pulse specifically for hummingbirds when attacking
            r_vals = []
            c_vals = []
            is_hmbg = self.sim.is_territorial[idx].item()

            for frame_idx in range(self.cfg.frames):
                 is_attacking = self.sim.active_defense_history[frame_idx][idx]
                 if is_attacking:
                      r_vals.append("12.0") # Swell in size when angry
                      c_vals.append("#ffffff") # Flash bright white
                 else:
                      r_vals.append("4.5")
            ...
            # Trail rendering
            if idx % 6 == 0 or is_hmbg: # Show more hummingbird trails to highlight their dominance
                d_path = "M " + " L ".join([f"{p[idx,0]:.1f},{p[idx,1]:.1f}" for p in self.sim.trajectory_history[::4]])
                stroke_w = 2.0 if is_hmbg else 1.0
                stroke_op = 0.5 if is_hmbg else 0.2
            ...
            if is_hmbg:
                 svg.append(f'<animate attributeName="r" values=\"{r_str}\" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
...
def main():
    ...
    print(f"Initializing territoriality simulator on {CONFIG.device}...")
    ...
    save_svg_to_drive(svg_content, 'notebook_09')
