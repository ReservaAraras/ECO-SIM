...
# MODULE 23: Carrying Capacity (Population Control)
# ===================================================================================================
"""
Module documentation.
Notebook Differentiation:
- Differentiation Focus: Carrying Capacity (Population Control) emphasizing tourism disturbance rhythms.
- Indicator species: Paca (Cuniculus paca).
...
class SimulationConfig:
    ...
    frames: int = 420 # Prompt requirement: 420 frames
    fps: int = 10
...
class TerraRoncaEcosystem:
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        self.population_history = [] # For plotting
    ...
    def step(self, frame_idx: int):
        ...
        if dead_mask.any():
            ...
            for idx in dead_idx:
                ...
                self.death_events.append({"pos": self.pos[idx].cpu().numpy().copy(), "frame": frame_idx, "color": self.colors[idx], "reason": reason})

        active_mask = self.is_active.clone()
        surface_mask = active_mask & ~self.is_underground
        current_pop = active_mask.sum().item()

        if not active_mask.any():
            self._log_history(current_pop)
            return
        ...
        fruiting_intensity = max(0.0, math.sin(fruit_cycle * 2 * math.pi))
        ...
        if can_grab_mask.any():
            ...
            if successful_grabs.any():
                ...
                self.seed_timers[successful_grabs] = torch.randn(self.cfg.max_particles, device=self.dev)[successful_grabs] * 5.0
        ...
        # NEW: REPRODUCTION LOGIC W/ CARRYING CAPACITY
        ...
        clamped_diff = torch.clamp(((new_angles - old_angles + math.pi) % (2 * math.pi) - math.pi)[surface_mask], -self.max_turns[surface_mask], self.max_turns[surface_mask])
        ...
        if surface_mask.any():
             ...
             clamped_diff = torch.clamp(((new_angles - old_angles + math.pi) % (2 * math.pi) - math.pi)[surface_mask], -dynamic_max_turns, dynamic_max_turns)
    ...
    def _log_history(self, current_pop):
    ...
    def generate_svg(self) -> str:
        ...
        if len(final_fire_nodes) > 0:
             ...
             for i, fn in enumerate(final_fire_nodes):
                  spawn_f = birth_frames[i] if birth_frames[i] != -1 else self.cfg.frames
        ...
        for seed_data in self.sim.dropped_seeds:
             ...
             op_vals, col_vals, rad_vals = [], [], []
             for f_idx in range(self.cfg.frames):
                  if f_idx < s_frame: op_vals.append("0.0"); col_vals.append("#ffe43f"); rad_vals.append("3.5")
                  elif f_idx < s_frame + self.cfg.seed_germination_delay: op_vals.append("1.0"); col_vals.append("#ffe43f"); rad_vals.append("3.5")
                  else: op_vals.append("1.0"); col_vals.append("#4caf50"); rad_vals.append("6.0")
        ...
        for birth in self.sim.birth_events:
             bpos = birth["pos"]; bframe = birth["frame"]
        ...
        for death in self.sim.death_events:
            dpos = death["pos"]; dframe = death["frame"]
            op_vals = ["0.0"] * self.cfg.frames
            for i in range(dframe, min(dframe+30, self.cfg.frames)): op_vals[i] = f"{max(0.0, 1.0 - (i-dframe)/30.0):.2f}"
            d_col = "#616161" if death["reason"] == "old_age" else "#d32f2f"
            svg.append(f'<g>')
        ...
        for idx in range(self.cfg.max_particles):
            if not any(self.sim.active_history[f][idx] for f in range(self.cfg.frames)): continue
            ...
            if idx % 8 == 0 or idx >= self.cfg.initial_particles:
                 ...
                 for frame_idx in range(0, self.cfg.frames, 2):
                      ...
                      if is_v_act: current_chunk.append(self.sim.trajectory_history[frame_idx][idx])
                      elif len(current_chunk) > 1: surface_chunks.append(current_chunk); current_chunk = []
                 if len(current_chunk) > 1: surface_chunks.append(current_chunk)
            ...
            if is_frugivore:
                 ...
                 svg.append(f'<animate attributeName="cx" values=\"{path_x}\" dur="{dur}s" repeatCount="indefinite" />')
                 svg.append(f'<animate attributeName="cy" values=\"{path_y}\" dur="{dur}s" repeatCount="indefinite" />')
                 svg.append(f'<animate attributeName="opacity" values=\"{";".join(seed_indicator_op_vals)}\" dur="{dur}s" repeatCount="indefinite" calcMode="discrete"/>')
                 svg.append('</circle>')

        # UIs
        svg.append(f'<text x="20" y="30" font-size="15" fill="#ffffff" font-weight="bold">ECO-SIM: Carrying Capacity Control</text>')
        svg.append(f'<text font-weight="bold" x="20" y="50" font-size="15" fill="#b0bec5">Systematic Population Caps</text>')

        # NEW: Population History Chart
        chart_w = 200
        chart_h = 60
        chart_x = 20
        chart_y = 70
        ...
        # Max pop limit line
        limit_y = chart_y + chart_h - (self.cfg.carrying_capacity / self.cfg.max_particles) * chart_h
        svg.append(f'<line x1="{chart_x}" y1="{limit_y}" x2="{chart_x+chart_w}" y2="{limit_y}" stroke="#ff4c4c" stroke-dasharray="4" stroke-width="1.5"/>')
        svg.append(f'<text font-weight="bold" x="{chart_x+chart_w+5}" y="{limit_y+4}" font-size="15" fill="#cccccc">Cap: {self.cfg.carrying_capacity}</text>')

        # Draw pop graph
        pop_pts = []
        for fi, p in enumerate(self.sim.population_history):
             px = chart_x + (fi / self.cfg.frames) * chart_w
             py = chart_y + chart_h - (p / self.cfg.max_particles) * chart_h
             pop_pts.append(f"{px:.1f},{py:.1f}")
        svg.append(f'<polyline points="{" ".join(pop_pts)}" fill="none" stroke="#00ffdc" stroke-width="2"/>')

        # Cursor for graph over time
        cx_vals = ";".join([f"{chart_x + (fi/self.cfg.frames)*chart_w:.1f}" for fi in range(self.cfg.frames)])
        cy_vals = ";".join([f"{chart_y + chart_h - (p/self.cfg.max_particles)*chart_h:.1f}" for p in self.sim.population_history])
        svg.append(f'<circle r="3" fill="#ffffff">')
        svg.append(f'<animate attributeName="cx" values="{cx_vals}" dur="{dur}s" repeatCount="indefinite" />')
        svg.append(f'<animate attributeName="cy" values="{cy_vals}" dur="{dur}s" repeatCount="indefinite" />')
        svg.append('</circle>')
...
def main():
    ...
    print(f"Initializing Carrying Capacity simulator on {CONFIG.device}...")
    sim = TerraRoncaEcosystem(CONFIG)
    for frame_idx in range(CONFIG.frames):
        sim.step(frame_idx)

    max_pop = max(sim.population_history)
    print(f"Simulation complete. Max Pop: {max_pop}, Final Pop: {sim.population_history[-1]}. Generating SVG...")

    renderer = EcosystemRenderer(CONFIG, sim)
    svg_content = renderer.generate_svg()
    display(HTML(svg_content))
    save_svg_to_drive(svg_content, 'notebook_23')
